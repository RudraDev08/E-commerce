...
app.get('/internal/metrics', async (req, res) => {
    try {
        ...
        // Step 1.1: Safe Division using ternary
        const calcRate = (numerator, denominator) => {
            return (denominator && denominator > 0) ? (numerator / denominator) * 100 : 0;
        };

        let stockDriftCount = 0;
        if (VM && IM) {
            const [variantCount, inventoryCount] = await Promise.all([
                VM.countDocuments({ status: { $ne: 'ARCHIVED' } }),
                IM.countDocuments({ isDeleted: { $ne: true } }),
            ]);
            stockDriftCount = Math.abs(variantCount - inventoryCount);
        }

        // Real calculations using globals generated by services
        const totalWrites = global.__totalWrites || 1;
        const totalAllocations = global.__totalAllocations || 1;
        const totalTxAttempts = global.__totalTxAttempts || 1;
        const totalCheckouts = global.__totalCheckouts || 1;
        const baselineReservations = global.__baselineReservations || 1;

        // Numerators mapped to rates
        const occConflictRate = calcRate(global.__occConflicts || 0, totalWrites);
        const allocationFailureRate = calcRate(global.__allocFailures || 0, totalAllocations);
        const transactionRetryRate = calcRate(global.__txRetries || 0, totalTxAttempts);
        const priceMismatchRate = calcRate(global.__priceMismatches || 0, totalCheckouts);
        const reservationSpikeRate = calcRate(global.__reservationSpikesCurr || 0, baselineReservations);
        const orderIdCollisions = global.__orderIdCollisions || 0;
        const promotionConflicts = global.__promotionConflicts || 0;

        const sysHealth = {
            stock_drift_count: stockDriftCount,
            occ_conflict_rate: parseFloat(occConflictRate.toFixed(2)),
            reservation_spike_rate: parseFloat(reservationSpikeRate.toFixed(2)),
            allocation_failure_rate: parseFloat(allocationFailureRate.toFixed(2)),
            transaction_retry_rate: parseFloat(transactionRetryRate.toFixed(2)),
            price_mismatch_rate: parseFloat(priceMismatchRate.toFixed(2)),
            order_id_collisions: orderIdCollisions,
            promotion_conflicts: promotionConflicts
        };

        // STEP 11: Automatic Freeze Protection
        // If there are collisions or stock drifts, we instantly freeze checkouts
        if (stockDriftCount > 0 || orderIdCollisions > 0) {
            global.systemState.checkoutFrozen = true;
            if (AlertLog) {
                await AlertLog.create({
                    metric: stockDriftCount > 0 ? 'stock_drift_count' : 'order_id_collisions',
                    value: stockDriftCount > 0 ? stockDriftCount : orderIdCollisions,
                    severity: 'CRITICAL',
                    details: { autoFrozen: true }
                }).catch(() => null);
            }
        }

        // Step 1.2: Plaintext output format (Prometheus format)
        res.set('Content-Type', 'text/plain');
        const textLines = Object.entries(sysHealth).map(([k, v]) => `${k} ${v}`);
        return res.send(textLines.join('\n'));
