...
const StateTable = () => {
  ...
  const add = async () => {
    if (!name.trim()) {
      showNotification("Please enter a state name", "error");
      return;
    }
    ...
    try {
      ...
      showNotification("Failed to add state", "error");
    } finally {
  ...
  const saveEdit = async (id) => {
    if (!editName.trim()) {
      showNotification("State name cannot be empty", "error");
      return;
    }
  ...
  const toggleActive = async (s) => {
    try {
      await updateState(s.id, { active: !s.active });
      await loadStates();
      showNotification(
        `${s.name} marked as ${!s.active ? "active" : "inactive"}`,
        "success"
      );
    } catch {
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
                  >
                    {actionLoading ? (
                      <>
            ...
            {countryId && (
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                ...
                <div className="space-y-3">
                  <button
                    onClick={loadStates}
                    className="w-full px-4 py-2 bg-blue-100 text-blue-700 hover:bg-blue-200 rounded-lg flex items-center justify-center gap-2 transition-colors"
                  >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none w-full sm:w-48"
                        />
                      </div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none appearance-none w-full sm:w-auto"
                        >
                            ...
                            {editId === state.id ? (
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none w-full"
                                  onKeyPress={(e) => e.key === "Enter" && saveEdit(state.id)}
                                  autoFocus
                                />
                              </div>
                            ) : (
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className={`h-6 w-11 rounded-full transition-colors duration-200 flex items-center p-1 ${
                                  state.active ? 'bg-green-500 justify-end' : 'bg-gray-300 justify-start'
                                }`}
                              >
                                <div className="h-4 w-4 bg-white rounded-full shadow-sm" />
                              </button>
                              <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                                state.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                              }`}>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between text-sm text-gray-500">
                    ...
                    {(searchTerm || filterActive !== 'all') && (
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-800 font-medium transition-colors mt-2 md:mt-0"
                      >
                        Clear filters
                      </button>
                    )}
