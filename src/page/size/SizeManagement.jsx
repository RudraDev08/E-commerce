...
const SizeManagement = () => {
    ...
    const [sortOrder, setSortOrder] = useState('priority'); // 'priority', 'name', 'newest'
    ...
    // --- UTILS ---
    const getSizeType = (name, code) => {
        const text = (name + " " + code).toUpperCase();
        if (text.includes('GB') || text.includes('TB') || text.includes('MB')) return 'storage';
        if (text.includes('UK') || text.includes('US') || text.includes('EU') || text.includes('CM')) return 'shoe';
        const apparelKeys = ['S', 'M', 'L', 'XL', 'XXL', 'XS', 'XXS', '3XL', 'SMALL', 'MEDIUM', 'LARGE'];
        if (apparelKeys.some(k => text.split(/[\s-]+/).includes(k))) return 'apparel';
        return 'generic';
    };

    const getTypeConfig = (type) => {
        switch (type) {
            case 'storage': return { icon: ServerIcon, bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-100', label: 'Storage' };
            case 'shoe': return { icon: TrophyIcon, bg: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-100', label: 'Shoe' };
            case 'apparel': return { icon: TagIcon, bg: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-100', label: 'Apparel' };
            default: return { icon: TagIcon, bg: 'bg-slate-50', text: 'text-slate-500', border: 'border-slate-100', label: 'Generic' };
        }
    };
        ...
        <div className="min-h-screen bg-slate-50/50 pb-12 font-sans text-slate-900">
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Size Master</h1>
                        <p className="text-sm text-slate-500 font-medium">Manage generic and category-specific units</p>
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    {/* Sort */}
                    <button
                        onClick={() => setSortOrder(prev => prev === 'priority' ? 'newest' : prev === 'newest' ? 'name' : 'priority')}
                ...
                <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
                    <table className="w-full">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Size Name / Type</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Code</th>
                                <th className="px-6 py-4 text-right text-xs font-bold text-slate-400 uppercase tracking-wider">Sort Val</th>
                            ...
                            {loading ? (
                                ...
                                [1, 2, 3, 4].map(i => (
                                    <tr key={i} className="animate-pulse">
                                        ...
                                        <td className="px-6 py-4"><div className="h-6 bg-slate-100 rounded-full w-12 mx-auto"></div></td>
                                ...
                                sizes.map((size) => {
                                    const typeConfig = getTypeConfig(getSizeType(size.name, size.code));
                                        ...
                                        <tr key={size._id} className="group hover:bg-slate-50/80 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${typeConfig.bg} ${typeConfig.border}`}>
                                                        <Icon className={`w-5 h-5 ${typeConfig.text}`} />
                                                    </div>
                                                    <div>
                                                        <div className="font-bold text-slate-900">{size.name}</div>
                                                        <div className={`text-[10px] font-bold uppercase tracking-wide inline-block mt-0.5 ${typeConfig.text}`}>
                                                            {typeConfig.label}
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td className="px-6 py-4">
                                                <button
                                                    onClick={() => handleCopyCode(size.code)}
                                                    className="group/code flex items-center gap-2 px-3 py-1.5 rounded-md bg-white border border-slate-200 shadow-sm text-xs font-mono font-bold text-slate-600 hover:border-indigo-300 hover:text-indigo-600 hover:shadow-md transition-all active:scale-95"
                                                    title="Click to copy"
                                                >
                                                    {size.code}
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <span className="font-mono text-slate-400 text-sm">{size.value || size.priority || '-'}</span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <button
                                                    ...
                                                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${size.status === 'active' ? 'bg-emerald-500' : 'bg-slate-200'}`}
                                                >
                                                    <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${size.status === 'active' ? 'translate-x-5' : 'translate-x-0'}`} />
                                                </button>
                                            </td>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all scale-100">
                        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-0.5">Define unit properly for variants</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. Extra Large, 128 GB"
                                    required
                                    autoFocus
                            ...
                            <div className="grid grid-cols-2 gap-5">
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">
                                        Code (Slug) <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        value={formData.code}
                            ...
                            <div className="flex gap-3 pt-6 border-t border-slate-100 mt-6">
                                ...
                                >
                                    {modalMode === 'create' ? 'Create Size' : 'Update Size'}
                                </button>
                            </div>
                        </form>
                    </div>
